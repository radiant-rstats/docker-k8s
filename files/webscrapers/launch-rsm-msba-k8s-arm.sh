...
if [ "${has_docker}" == "" ]; then
  ...
  show_service () {
    ...
    if [ -z "${menu_exec}" ]; then
      ...
      # Build custom Playwright HTTP server if not exists
      if [[ "$(docker images -q playwright-http-server 2> /dev/null)" == "" ]]; then
        echo "Building custom Playwright HTTP server..."
        current_dir=$(pwd)
        script_dir="$(dirname "${BASH_SOURCE[0]}")"
        cd "${script_dir}/files/webscrapers" 2>/dev/null || cd "${script_dir}"
        
        if [ -f "Dockerfile.playwright" ]; then
          docker build -t playwright-http-server -f Dockerfile.playwright .
        else
          echo "❌ Dockerfile.playwright not found. Using default Playwright image."
          PLAYWRIGHT_IMAGE="mcr.microsoft.com/playwright:latest"
        fi
        cd "${current_dir}"
      else
        PLAYWRIGHT_IMAGE="playwright-http-server"
      fi
      
      playr_nr=($(docker ps -a | awk "/rsm-playwright/" | awk '{print $1}'))
      playr_nr=${#playr_nr[@]}
      if [ "$CPORT" != "" ]; then
        echo "A Playwright container may already be running on port ${playr_port}"
        playr_nr=$((${playr_nr}-1))
      else
        if [ "${PLAYWRIGHT_IMAGE}" == "" ]; then
          PLAYWRIGHT_IMAGE="playwright-http-server"
        fi
        docker run --name="rsm-playwright${playr_nr}" --net ${NETWORK} -d -p 127.0.0.1:${playr_port}:3000 --platform linux/arm64 ${PLAYWRIGHT_IMAGE}
      fi
