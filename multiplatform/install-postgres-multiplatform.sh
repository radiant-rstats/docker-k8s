...
# Modified to avoid starting services during build
...
# Only start PostgreSQL if not in build mode
if [ "$POSTGRES_NO_START" != "1" ]; then
    ...
    echo "Skipping PostgreSQL startup during build (POSTGRES_NO_START=1)"
    echo "Database and user creation will be handled at container startup"
fi
